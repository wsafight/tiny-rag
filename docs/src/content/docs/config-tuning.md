...
`TOP_K`、`CHUNK_SIZE` 和上下文长度要一起看：`TOP_K` 越大，进入 Prompt 的片段越多；`CHUNK_SIZE` 越大，单个片段越长。两者同时变大时，模型会看到更多内容，但不一定看到更干净的证据。先保证正确片段能进入 `hits`，再逐步压掉无关上下文。
...
:::note[下一章：诊断与解析方法]
配置讲完了，下一章把一次失败查询拆成 `documents -> chunks -> candidates -> hits -> context -> answer`，用中间结果判断问题到底落在导入、召回还是生成。
:::
