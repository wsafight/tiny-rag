...
建议准备一组固定问题，每次改参数后都跑一遍：

| 问题类型 | 目的 |
| --- | --- |
| 明确有答案的问题 | 验证正确 source 能否排前 |
| 同义改写问题 | 验证向量语义召回 |
| 含专有名词/编号的问题 | 验证关键词和 BM25 |
| 跨多个文档的问题 | 验证 `TOP_K` 和同源限制 |
| 明确无答案的问题 | 验证 `MIN_SCORE` 和 unknown answer |
...
这个小表比“感觉变好了”可靠得多。RAG 调参要避免凭单次回答判断效果。

## RAG 评测的最小维度

诊断是看一次失败查询，评测是看一组固定问题的整体表现。哪怕不做复杂自动化，也建议至少按四个维度记录结果：

| 维度 | 看什么 | 为什么重要 |
| --- | --- | --- |
| source 命中 | 正确资料是否进入 `candidates` / `hits` | 判断召回是否有效 |
| 排序位置 | 正确片段排第几，是否被同源限制挤掉 | 判断排序和 TopK 是否合理 |
| 答案边界 | 回答是否只使用 context 里的证据 | 判断 Prompt 和生成是否可靠 |
| 无答案拒答 | 知识库没有答案时是否回答 unknown | 判断 `MIN_SCORE` 和兜底策略 |

这四个维度比“回答看起来顺不顺”更接近 RAG 的真实质量。一个回答写得很流畅，但 source 不对，仍然是失败；一个回答略短，但引用正确、边界清楚，反而更可靠。
