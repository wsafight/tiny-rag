...
```text
召回 recall：先拿到一批可能相关的 candidates
排序 ranking：用向量分、BM25 分或融合分给 candidates 排序
重排 rerank：用更贵的模型或规则重新判断候选之间的先后
```

tiny-rag 当前做到的是“召回 + 融合排序 + 同源限制”。如果以后加 reranker，它应该放在 `candidates` 之后、最终 `hits` 之前。reranker 只能改善“正确片段已经在候选池里，但排序不够好”的问题；如果正确片段根本没进 `candidates`，要回头看导入、切块、embedding、关键词分词或候选池大小。
...
export interface SearchOptions {
  /** 最终返回多少条命中。 */
  topK: number;
  /** 关键词分权重；0 表示纯向量，越大越重视字面命中。 */
  keywordWeight?: number;
  /** 同一个 source 最多进入多少条，避免单文件占满上下文。 */
  perSourceLimit?: number;
}

export function createRetriever(store: LoadedStore): Retriever {
  // 关键词索引和向量库一起被闭包复用，避免每次查询重复统计全库词频。
    ...
    search(questionEmbedding, questionText, opts) {
      ...
      const scored: SearchHit[] = raw.map((x) => {
        ...
        return {
          ...
          // 先对齐尺度，再按权重融合；kw=0.3 表示 70% 向量 + 30% 关键词。
...
function selectDiverseHits(sorted: readonly SearchHit[], topK: number, perSource: number): SearchHit[] {
  ...
  for (const hit of sorted) {
    ...
    // 当前 source 已达到上限时跳过，让其他来源有机会进入上下文。
    if (n >= perSource) continue;
    counts.set(hit.source, n + 1);
    out.push(hit);
    // 收够 topK 就停止，避免继续扫描无关候选。
