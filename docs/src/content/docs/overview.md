...
3. **工程化**：理解 CLI / HTTP / 库 API 三种入口，以及配置与检索调参。
4. **诊断与速查**：用 [诊断与解析方法](/tiny-rag/diagnostics/) 排查失败查询，遇到生词随时翻 [核心术语速查](/tiny-rag/glossary/)。
5. **优化与扩展**：看 tiny-rag 在简化版之上做了哪些优化，以及未来的 TODO。

建议先按顺序读 `B01` 到 `B10`。这十章是一个连续博客系列：每章引入一个新约束，并把它落成可运行代码。读完后再看“接口、配置、诊断、优化、扩展”，你会更容易判断哪些设计是为了学习简化，哪些是为了真实使用而补上的工程保护。
...
- 推荐读法：先认识（本章 + 架构），再跟 `B01`–`B10` 从零构建，然后看工程化、诊断速查与扩展。
