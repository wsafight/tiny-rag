...
npm run ingest                # 生成向量库 vector-store.ndjson
npm run query -- "如何取消订单？"
```
...
```bash
npm run ingest    # 读取 DOCUMENTS_DIR，写入 VECTOR_STORE
```

生成的 `vector-store.ndjson` 是本地向量库：第一行是元数据，后续每行是一条 chunk 记录。**修改资料后需要重新运行 `npm run ingest`。**
...
```bash
npm run serve     # 默认监听 http://127.0.0.1:8787
```
...
npm test             # Node test runner 测试
npm run build        # 构建库入口到 dist/
```
